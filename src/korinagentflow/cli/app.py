...
提供 `korinagentflow` 命令及其子命令。

用法:
    korinagentflow                              # 显示帮助
    korinagentflow test                         # LLM 连接测试
    korinagentflow test "自定义提示词"           # 使用自定义提示词测试
    korinagentflow tool-test -t calculator "2+3"  # 测试计算器
    korinagentflow tool-test -t bash "dir"        # 测试 Bash
    korinagentflow tool-test -t search "Python"   # 测试搜索
"""
...
# ⚠️ 必须在导入 korinagentflow 之前检测缓存并设置 HF_HUB_OFFLINE
# korinagentflow.providers 会导入 langchain_openai，可能触发 huggingface_hub 初始化
...
from korinagentflow.providers.openai_provider import create_model  # noqa: E402
...
def _test_bash(state: RuntimeState, command: str, timeout: int | None) -> None:
    """测试 BashTool。"""
    from korinagentflow.tools.bash_tool import run_bash
...
def run_command(
    ...
    示例:
        korinagentflow run "计算 123 * 456"
        korinagentflow run "搜索 Python 最新版本并总结"
        korinagentflow run "列出当前目录的文件"
        korinagentflow run "帮我查 LangGraph 是什么，然后算 2^10"
    """
...
def memory_test_command(
    ...
    使用方式:
        korinagentflow memory-test --add "我喜欢Python"
        korinagentflow memory-test --pref "偏好异步编程"
        korinagentflow memory-test --fact "项目使用LangGraph"
        korinagentflow memory-test --search "编程语言"
    """
    from korinagentflow.providers.openai_provider import create_model
    from korinagentflow.memory import MemoryManager
